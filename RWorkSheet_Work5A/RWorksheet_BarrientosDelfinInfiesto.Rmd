    ...
    if (!is.na(urls[i])) {
      ...
      if (!is.null(page)) {
        reviews_data <- page %>%
          ...
          html_text()
        results[[i]] <- if (length(reviews_data) > 0) reviews_data else NA
      }
    }
  }

  return(data.frame(Reviews = unlist(results, use.names = FALSE)))
}

electronics_reviews <- reviews_text(review_links_df[[1]]$review_links)
